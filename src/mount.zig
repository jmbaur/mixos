...
fd: union(FD) { fs: posix.fd_t, mnt: posix.fd_t },
attrs: usize = 0,
...
fn fsconfig(fd: posix.fd_t, cmd: usize, key: ?[*:0]const u8, value: ?[*:0]const u8, aux: usize) Error!void {
    switch (system.E.init(system.syscall5(
        .fsconfig,
        @bitCast(@as(isize, fd)),
        cmd,
        @intFromPtr(key),
...
pub fn init(fstype: [*:0]const u8) Error!Mount {
    return .{ .fd = .{ .fs = try fsopen(fstype) } };
}

pub fn initTree(dir: std.fs.Dir, path: []const u8) Error!Mount {
...
pub fn finish(self: *Mount, dest_dir: std.fs.Dir, dest: [*:0]const u8, attrs: usize) Error!void {
    const mntfd = switch (self.fd) {
        .fs => |fsfd| b: {
    ...
    defer posix.close(mntfd);

    try move_mount(mntfd, "", dest_dir.fd, dest, C.MOVE_MOUNT_F_EMPTY_PATH);
